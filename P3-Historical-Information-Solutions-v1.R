...
#### Change of weight to 50%
# Robustification of MAP Prior
rmap  = robustify(map, weight=0.5, mean=-50)
rmap
summary(rmap)
plot(rmap)
...
## then calculate probability for the criterion
## and compare to the predefined threshold values
p = pmixdiff(post_act, post_pbo, 0)
print(p>0.975)
round(p*100,1)
...
# Change of data with large drift
# Robustification of MAP Prior
rmap  = robustify(map, weight=0.2, mean=-50)
rmap
summary(rmap)
...
##########################################################################
##  Point (6): Operating characteristics
##########################################################################
...
##########################################################################
##  Point (7): Operating characteristics
##########################################################################
